  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-background px-5 py-6 md:px-8 md:py-8">
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-5 grid gap-5 md:grid-cols-[0.8fr_1.2fr]"><div className="glass rounded-3xl p-6 md:p-8"><h2 className="font-display text-2xl font-semibold">Technologies</h2><div className="mt-5 flex flex-wrap gap-2">{technologies.map((tech) => <span key={tech} className="rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-sm text-primary">{tech}</span>)}</div><a href="https://archivio-s-project-oyaa9xbwj-archiviovault-3657s-projects.vercel.app/" target="_blank" rel="noreferrer" className="mt-8 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm text-muted-foreground transition hover:-translate-y-0.5 hover:text-foreground">RitzCredentials <ArrowUpRight className="size-4" /></a></div><div className="glass rounded-3xl p-6 md:p-8"><h2 className="font-display text-2xl font-semibold">Screenshots & visuals</h2><div className="mt-5 flex min-h-40 items-center justify-center rounded-2xl border border-dashed border-white/15 bg-white/[0.03] text-center text-sm text-muted-foreground">Live project preview available.</div></div></section>
      </div>
    </main>
  )
}
