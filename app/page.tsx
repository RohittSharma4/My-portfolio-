...
export default function Page() {
  const [activeProject, setActiveProject] = useState<'ritzcredentials' | 'offline-mesh-messenger' | null>(null)

  if (activeProject) {
    return <ProjectDetailOverlay project={activeProject} onBack={() => { setActiveProject(null); requestAnimationFrame(() => document.getElementById('projects')?.scrollIntoView({ behavior: 'instant' })) }} />
  }
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background pt-24 md:pt-0">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {projects.map((project, index) => (
              <Reveal key={project.title} delay={index * 100} as="article" className="group relative overflow-hidden rounded-3xl border border-border bg-card p-6 transition-all duration-500 hover:-translate-y-2 hover:border-primary/40 hover:shadow-[0_20px_70px_-30px_oklch(0.65_0.19_258_/_0.7)]">
                {project.href ? (
                  <button type="button" onClick={() => setActiveProject(project.href === '/projects/ritzcredentials' ? 'ritzcredentials' : 'offline-mesh-messenger')} className="block h-full w-full rounded-2xl text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={`View ${project.title} project introduction`}>
      ...
      <section id="certifications" className="scroll-mt-24 py-24 md:py-32"><div className="mx-auto max-w-6xl px-5 md:px-6"><Reveal><SectionHeading eyebrow="Certifications & achievements" title="Proof of curiosity." /></Reveal><Reveal delay={100} as="article" className="glass group mt-10 overflow-hidden rounded-3xl p-5 transition-all duration-500 hover:-translate-y-1 hover:border-primary/40 md:p-7"><div className="flex flex-col gap-7 sm:flex-row sm:items-center"><div className="flex size-40 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/5 p-4 shadow-xl sm:size-44"><img src="/certificates/nasa-open-science-essentials.png" alt="NASA Open Science Essentials badge" width="160" height="160" className="size-32 object-contain transition-transform duration-500 group-hover:scale-105 sm:size-36" /></div><div className="min-w-0"><div className="flex items-center gap-2 text-primary"><Award className="size-4" /><span className="text-xs font-medium uppercase tracking-[0.22em]">Completed course</span></div><h3 className="mt-3 font-display text-2xl font-semibold md:text-3xl">NASA Open Science Essentials</h3><p className="mt-3 max-w-2xl leading-relaxed text-muted-foreground">Completed the NASA Open Science Essentials course and earned this achievement badge.</p><a href="/certificates/OpenScienceEssentials20260819-20-9aoqtr.pdf" target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-medium text-primary-foreground transition-transform hover:-translate-y-0.5"><span className="sr-only">Certificate: </span>View Certificate <ArrowUpRight className="size-4" /></a></div></div></Reveal></div></section>
